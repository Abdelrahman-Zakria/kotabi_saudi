...
import 'package:kotabi_saudi/core/services/hululi_catalog_service.dart';
import 'package:kotabi_saudi/features/new_ui/widgets/book_cover_image.dart';
import '../browse/subject_books_screen.dart';

class HululiBooksScreen extends StatefulWidget {
  final GradeModel grade;

  const HululiBooksScreen({super.key, required this.grade});

  @override
  State<HululiBooksScreen> createState() => _HululiBooksScreenState();
}

class _HululiBooksScreenState extends State<HululiBooksScreen> {
  final HululiCatalogService _service = HululiCatalogService.instance;
        ...
        slivers: [
          ...
          FutureBuilder<List<BookModel>>(
            ...
            builder: (context, snapshot) {
              ...
              final List<BookModel> books = snapshot.data ?? const <BookModel>[];
              
              final List<BookModel> queryFiltered = _applyQuery(books, _query);
              
              // Force use of 1st and 2nd semesters only
              final List<BookModel> filtered = queryFiltered.where((book) {
              ...
              final List<String> subjects = filtered
  ...
  List<BookModel> _applyQuery(List<BookModel> booksList, String query) {
    final trimmed = query.trim();
    if (trimmed.isEmpty) return booksList;
