...
class MnhajiSourceCatalogService {
  final String _assetPath;

  MnhajiSourceCatalogService._(this._assetPath);

  static final MnhajiSourceCatalogService manhajiBooks =
      MnhajiSourceCatalogService._('assets/data/كتب_منهجي.json');

  static final MnhajiSourceCatalogService examModels =
      MnhajiSourceCatalogService._('assets/data/نماذج_الاختبارات.json');
  ...
  Future<void> initialize() async {
    ...
    final items = (decoded['items'] as List<dynamic>? ?? const [])
        ...
        .where(
            (item) => item.id.trim().isNotEmpty && item.grade.trim().isNotEmpty)
        .toList();
  ...
  Future<List<MnhajiGradeEntry>> fetchGrades() async {
    await initialize();
...
class MnhajiGradeEntry {
  final GradeModel grade;
  final int count;

  const MnhajiGradeEntry({
    required this.grade,
    required this.count,
  });
}
